...
def darken(pixel: tuple, magnitude: int) -> tuple:
    """Decreases the brightness of a pixel
    Args:
        pixel: a 3-tuple of (red, green, blue)
            subpixels
        magnitude: an int from 0-255 that
    ...
    # increase the value by some number
    if red + magnitude < MIN:
        red = MIN
    else:
        red += magnitude
    if green + magnitude < MIN:
        green = MIN
    else:
        green += magnitude
    if blue + magnitude < MIN:
        blue = MIN
    else:
        blue += magnitude
...
for y in range(image_height):
    ...
    for x in range(image_width):
        ...
        brighter_pixel = darken(pixel, -100)
